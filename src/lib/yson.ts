...
const u = undefined;
const sI: any = (setTimeout as any).turn || (typeof setImmediate !== 'undefined' ? setImmediate : setTimeout) || setTimeout;

const yson: any = {};

yson.parseAsync = function (text: any, done: any, revive?: any, M?: any): void {
  if ('string' != typeof text) { try { done(u, JSON.parse(text)) } catch (e) { done(e) } return }
  var ctx: any = { i: 0, text: text, done: done, l: text.length, up: [] };
  M = M || 1024 * 32;
  parse();
  function parse(): void {
    var s = ctx.text;
    var i = ctx.i, l = ctx.l, j = 0;
    var w = ctx.w, b: any, tmp: any;
    while (j++ < M) {
      var c = s[i++];
      if (i > l) {
        ctx.end = true;
      ...
      if (w) {
        ...
        if ('"' == c && !tmp) {
          ...
          if (ctx.a) {
            ...
            if (b || (1 + tmp.indexOf('\\'))) { tmp = JSON.parse('"' + tmp + '"') }
            ...
            if (b || (1 + ctx.s.indexOf('\\'))) { ctx.s = JSON.parse('"' + ctx.s + '"'); }
          }
          ctx.a = b = u;
        }
        ++i;
    ...
    if (ctx.end) {
      tmp = ctx.at;
      if (u === tmp) {
        try { tmp = JSON.parse(text) } catch (e) { return ctx.done(e) }
      }
      ctx.done(u, tmp);
    } else {
      sI(parse);
...
yson.stringifyAsync = function (data: any, done: any, replacer?: any, space?: any, ctx?: any): void {
  ctx = ctx || {};
  ctx.text = ctx.text || "";
  ...
  ify();
  function ify(): void {
    var at = ctx.at, data = at.d, add: any = '', tmp: any;
    if (at.i && (at.i - at.j) > 0) { add += ',' }
    if (u !== (tmp = at.k)) { add += JSON.stringify(tmp) + ':' }
    switch (typeof data) {
      case 'boolean':
        add += '' + data;
        break;
